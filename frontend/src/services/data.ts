...
export const saveNote = async (user_id: string, pattern: string, variant_id: string, note: string, current_session_user: string) => {
  try {
    // API 요청 데이터 검증
    if (!user_id || !variant_id || !current_session_user) {
      throw new Error('Required parameters missing: user_id, variant_id, current_session_user');
    }
